...
| CTs | (CT=Control Transfer) The number of deviations from sequential control flow instructions (typically taken branches) that occured among the instructions that retired in the cycle the sample was taken. Breaks from control flow can include interrupts and exceptions.
| CToffset1 | If CTs > 0, this field records offset from the oldest retiring sampled PC (which is stored in `shpmspc`) at which the first branch occured. The PC of the branch target will be available in the first field of `shpmsdetailsbtpcs`.
| CToffset2 | If CTs > 1, this field records offset from the oldest retiring sampled PC (which is stored in `shpmspc`) at which the second branch occured. The PC of the branch target will be available in the second field of `shpmsdetailsbtpcs`.
| RState | The state of the retire stage in the cycle the sample was taken. `00` signifies that the core committed at least one instruction, `01` means that it is stalled, `10` reports that retire is empty due to a pipeline flush, and `11` reports that retire is empty because it was not supplied with instructions.   
| Cause0,1,2,3 | The implementation uses these fields to provide further information about the retire stage for I0, I1, I2, and I3 of multiple instructions retiring in the sampled cycle, e.g. the (combinations of) performance event(s) the retiring instructions have been subjected to. This spec recommends fixing the first set of Cause bits to be I$miss, ITLBmiss, Branch mispredict, D$miss, DTLBmiss
|====

As briefly described above, sampling software can calculate the addresses of the retiring instructions by assuming sequential execution starting from the PC in `shpmspc`. This however does not take into account that there may be taken branches or other control transfers among the retiring instructions. The `CTs` field thus reports the number of taken branches among the retiring instructions. If `CTs`=0, sampling software knows that the sequential execution assumption holds. If `CTs`=1 (`CTs`=2), the instruction stream jumped to the first address in `shpmsdetailsbt1` at the offset recorded in the `CToffset1` (`CToffset2`) field. If `CTs`>2, attribution will be biased since only two branch targets can be communicated. If this is a concern, implementors should consider supporting CTR because it can be used to identify all taken branches. 

Alternatively, this sampling hardware is designed to handle a max of Control Transfers (CTs) per sample; any more CTs occur and sampling software stops assigning execution times to instructions following the second CT.  This could make the results slightly imprecise, however it is likely that > 2 CTs retiring in one cycles will occur infrequently.

The purpose of the `RState` and `Causes` fields is to enable creating Per-Instruction Cycle Stacks (PICS). PICS break down the time attributed an instruction to explain its impact on overall execution time. One way to create PICS is to break down each instruction's contribiution to overall execution time according to state of the retire stage in each sample, and this use case is supported by the `RState` field. `RState` is a two bit field because there are four fundamental retire states, i.e., committing, stalling, drained, or flushed. Another option is to break down time according to the (combinations of) performance events the instruction was subjected to, which is supported by the `Causes` field. The tracked performance events will be implementation specific.

For the non-committing sample states (stalling, draining, flushed), hardware may use the 'shpmspc' "Sample PC register" to point to the instruction that caused the appropriate state.

NOTE: _Even if the meaning of the causes field is implementation specific, the software required for creating PICS can be reused across implementations. The reason is that creating PICS requires partioning samples according to the values of the `RState` or `Causes` fields, and this task is independent of the specific meaning of each value._
...
|====
| Field | Description 
| AR | Sampling goes into Autorun mode when the AR bit is set to 1. Autorun is disabled when set to 0. 
|====


The `shpmsdetailscc` register enables streaming data to a memory buffer. For example if the output of Sspesa is connected to a trace unit, the hardware will take a sample and send it to the trace unit to be formed into a message and output. The value in the upper bits of `shpmsdetailscc` are set into a count-up cycle counter. The counter runs and when it overflows, a sample is taken, streamed to the trace, and the count-up cycle counter is reloaded with the contents of this register and started counting again.

This register could also be used to generate an overflow interrupt to flag software to read the CSR sample registers, thus not requiring a programmable performance counter to be dedicated for this use.

NOTE: _It is recommended that hardware is implemented to prevent values to be put into the register that cause a period shorter than 2^6 to 2^8 cycle counts, otherwise the downstream hardware may not be able to handle the data rates._
...
For RV64 implementations, bits 6:0 are hardcoded to 0, while for RV32 implementations, bits 5:0 are hardcoded to 0.  This ensures that records are aligned to their size, see <<samprec>> for details.
...
WARNING: _Do we need a discovery mechanism so software can know which optional fields are included?  When not implemented, they always report 0.  Could add a read-only CSR for this?  Or embed it in the record?_
...
The PDIS memory buffer is an instance of the Asynchronous Memory Buffer, as defined by the Ssamb extension, see Ch 1 of the https://github.com/riscv/self-hosted-trace/releases[Self-hosted Trace Specification].  The PDIS memory buffer (PMB) must be implemented if `mpdisctl`.MEM is not hardcoded to 0. All optionality available within the Ssamb extension is available for the PMB.
