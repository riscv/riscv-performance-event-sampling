...
Statistical (random) error is the second reason why Sspesa profiles are approximate. The root cause of statistical error is that Sspesa samples the its CSRs, and it is thus unavoidable. Statistical error is however typically negligible when sampling at 4kHz (which is the default sampling frequency of perf). Unlike bias, statistical error goes down as sampling frequency is increased. If statistical error is percieved to be a problem, a higher sampling frequency can be adopted at the cost of increased runtime overhead.

WARNING: I can provide references to papers that back up the negligible error claim above if desirable. --Magnus
