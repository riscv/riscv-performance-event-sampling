...
[NOTE]
====
_A physical address or guest physical address written to pdisadr2 is shifted right by 2 bits to accommodate addresses wider than the current XLEN. The Sv32 page-based virtual-memory scheme described in Section 12.3 supports 34-bit physical addresses for RV32, so pdisadr2 must support addresses wider than XLEN for RV32. This shift-by-2 encoding of physical and guest physical addresses matches the encoding of physical addresses in PMP address registers (Section 3.7) and in page table entries (Section 12.3, Section 12.4, Section 12.5, and Section 12.6).  If the least-significant two bits are needed, these bits are the same as the least-significant two bits of the virtual address in pdisadr1._
====
