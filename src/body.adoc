...
The `shpmsdetails` CSRs provide an optional extension targeted at implementations that want to provide metadata in addition to `shpmspc` and `shpmsdata` for collected samples. A key use case to (partially) account for instruction-level parallelism when the core does not support Control Transfer Records (SSctr). Another reason for supporting this CSR is that it provides a standard interface for communicating the reason(s) why an instruction was sampled to profiling software through a specified software interface.

The sample details mechanism consists of two additional SXLEN-bit read/write WARL registers called `shpmsdetailsbtpcs` and `shpmsdetailsdata`. The `shpmsdetailsbtpcs` is used stores the PC of up to two branch target address if a at least one taken branch occurs among the instructions that the core retired in the cycle the sample was taken. The rationale for supporting up two branches is that cores with higher retire widths (e.g., 8) will typically implement SSctr.
...
.Sample Details Branch Target PC Register for SXLEN=64  (`shpmsdetailsbt`)
[%unbreakable]
[wavedrom, , svg]
....
{reg: [
...
| CTs | The number of deviations from sequential control flow instructions (typically taken branches) that occured among the instructions that retired in the cycle the sample was taken. Breaks from control flow can include interrupts and exceptions.
...
| Causes | Implementations can use this field to provide further information about the retire stage in the sample the cycle was taken, e.g., the (combinations of) performance event(s) the retiring instructions have been subjected to. 
|====

Sampling software can estimate the addresses of the retiring instructions by assuming sequential execution starting from the PC in `shpmspc`. This will however be wrong when there are taken branches among the retiring instructions. The `CTs` field thus reports the number of taken branches among the retiring instructions. If `CTs`=0, sampling software knows that the sequential execution assumption holds. If `CTs`=1 (`CTs`=2), the instruction stream jumped to the first address in `shpmsdetailsbt` at the offset recorded in the `CToffset1` (`CToffset2`) field. If `CTs`>2, attribution will be biased since only two branch targets can be communicated. If this is a concern, implementors should consider supporting CTR because it can be used to identify all taken branches.
...
....
{reg: [
    {bits:  32, name: 'Cycle counter init'}
], config:{lanes: 1, hspace:1024}}
....

The `shpmsdetailscc` register enables streaming data to a memory buffer.

WARNING: Bruce: Please input text here explaining why we need `shpmsdetailscc` 
