...
| DSRC | Data source.  Encodings for this field are implementation-defined, though 0 implies that DSRC is not reported.
...
NOTE: _Some implementations may execute stores post-retirement, such that stores perform a read-for-ownership (RFO) only once they are non-speculative and deallocated from the ROB.  For such post-retirement RFOs, the latency scheme described above will not capture the latency of such post-retirement RFOs, nor will the load/store event information described in <<pdishdrldst>> be captured.  It is left to a future extension to add formal PDIS support for such implementations, if needed.  Until then, such implementations should record EXECV=1 and EXECUTION=0 for sampled post-retirement stores._
...
Batch mode is available only if `mpdisctl`.MEM=1.  To enable batch mode, PDIS is configured with `mpdisctl`.OF=1 and `spdisambcs`.BMIEN=1.  When a sampled instruction is qualified, a sample record will be stored to the PDIS Memory Buffer (<<membuff>>).  Once the buffer write pointer reaches the `spdisambcs`.BMITH threshold, a local asynchronous memory buffer interrupt (LAMBI) is pended with `spdisambcs`.BMI=1.  The LAMBI handler can then collect the records.
