...
Extensions such as Sspesa and Ssplcofi allow users to perform event-based sampling with precise attribution to an instruction PC.  Smpdis/Sspdis takes a different approach, sampling decoded instructions rather than performance events.  The sampled instructions are chosen in the frontend portion of the pipeline, allowing runtime metadata to be collected by the hardware as the sampled instruction passes through the backend pipeline.  So while event-based sampling with precise attribution allows attributing a single event (the event being sampled) with an instruction, Precise Decoded Instruction Sampling (PDIS) allows collection of multiple events, latencies, and addresses associated with the instruction's execution.  This paints a much more complete picture of the factors that contribute to the instruction's performance, and its impact on overall workload performance.
...
NOTE: _When to use event-based sampling (like Sspesa or Ssplcofi) vs instruction-based sampling (Smpids/Sspdis) is an art rather than a science.  Say more here._
...
|====
| Field | Description
| SEL | Selects the types of instructions to be counted and sampled.  Encodings are as follows.

0: All instructions decoded.

1: Load instructions decoded.

2: Store instructions decoded.

3: Load and store instructions decoded.

4: Control transfer instructions decoded.
...
| PTRG | Enable recording of the prior control transfer target.  See <<samprec>>.
| Custom | WARL bits designated for custom use.  The value 0 must correspond to standard behavior.  See <<Custom Extensions>>.
...
NOTE: _The PDIS counter is limited to 32 bits since, unlike Zihpm counters, it is used only for sampling, rather than for counting.  When Zihpm counters are used in counting mode they count all events for an application, which can run for minutes or more.  Sampling, on the other hand, requires counters only to count through the sample period, which is typically less than 10ms.  Shorter sample periods (higher sampling rates) are preferred for more accurate and detailed profiles, but minimizing sampling overhead can cause longer periods to be chosen.  With a 32-bit counter, even a 10-wide core running at 6 GHz could accommodate a sample period as long as 71ms._
...
|===
| `siselect` | Indirect Register | Record Field
.6+| TBD | `sireg` | pdishdrev[31:0]
| `sireg2` | pdishdrev[63:32]
| `sireg3` | pdispc
| `sireg4` | pdistime
| `sireg5` | pdislat[31:0]
| `sireg6` | pdislat[63:32]
.6+|TBD2 | `sireg` | pdisadr1
| `sireg2` | pdisadr2
| `sireg3` | _WPRI_
| `sireg4` | _WPRI_
| `sireg5` | _WPRI_
...
If the H extension is implemented, the same mappings above apply to `vsireg__i__` when `vsiselect` holds TBD or TBD2.

If `mpdisctl`.MEM is hardcoded to 1, the PDIS Sample Data registers are not implemented.
...
The `spdisfilter` register allows the user to select a sample data latency value (see <<pdislat>>) and compare it with a specified threshold, such that the comparison gates sample qualification.
...
|===
| Field | Description
| THRESH | The latency threshold to compare with the selected PDIS latency value (see <<PDIS Latencies>>).  The comparison evaluates to true if the latency value is greater than or equal to the threshold value.
...
Implementations may opt to hardwire some or all bits in `spdisfilter`.
...
NOTE: _It is strongly recommended that implementations avoid bias in sample selection.  Always choosing an instruction from decoder 0, for instance, could bias selection towards branch targets, or other instructions that are more likely to use decoder 0._
...
If a second address, aside from the PC, is captured for this sample, it is held in pdisadr2.  If no second address is captured for this sample, pdisadr2 is 0.
...
If the instruction was not flushed, the instruction is of type control transfer, and `mpdisctl`.PTRG=1, pdisadr2 holds the target of the prior control transfer instruction.
...
NOTE: _PDIS.SAMPLES in most cases will match the number of samples collected, but if a ciruclar memory buffer is used some samples may be overwritten.  The the total number of PDIS counter overflows can be inferred from PDIS.SAMPLES + PDIS.COLLISIONS + PDIS.FILTERED + PDIS.DROPPED.  The INST.*.SPEC events provide the full sample population from which the selections were chosen._
