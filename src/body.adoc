...
The Sspesa extension defines a mechanism by which the hart will, upon Zihpm counter overflow, record a PC and metadata associated with the instruction to which the sampled event should be attributed.  From this PC and metadata, and optionally other state, the attribution PC can be determined.  For select performance events precise attribution is supported, such that the derived PC is that of the instruction that incurred the event that caused the overflow.  The list of such events is implementation-defined.  For other performance events, the derived PC is the best PC for attribution that the implementation can provide.

NOTE: _The set of performance events that support precise attribution with Sspesa should be discoverable via the implementation's performance event JSON file.  Events with the 'PesaSupported' (TBD) attribute set to true support precise attribution._
...
* *Time-based performance profiles* in which the weight of an instruction in the profile is approximately proportional to its contribution to overall execution time. To obtain such a profile a counter is configured to sample on cycles. Upon overflow of the counter, Sspesa allows profiling software to approximate the instruction(s) whose exection time was exposed at the time the sample was taken. A sorted time-based profile orders the workload's instructions according to their approximate impact on overall execution time, and is hence most useful as a first-pass approach for identifying application hot spots.

NOTE: _In contrast to Sspesa, Smpdis/Sspdis (see <<smpdis>>) create profiles in which an instruction is represented proportionally to its execution count._
...
| MEM | Read-only bit that, when set, indicates that sample records (<<samprec>>) are written to the PDIS Memory Buffer (<<membuff>>).  When 0, sample records are written to `spdisdata__i__` registers (<<dataregs>>).
...
| FNC | Set to 1 by hardware if the qualified sample requires a FENCE before collection.
...
The M bit is always writable.  The S and U bits are writable if the corresponding privilege mode is implemented.  Otherwise, an implementation may opt to hardwire to 0 any of the other fields described above.  The MEM bit is always hardwired (to 1 or 0), while the ACC bit may be hardwired to 0 or 1.
...
If FNC=1, software should perform a FENCE instruction before collecting the sample record(s), to ensure that any sampled store is complete.
...
Bit 59 is PDISS.  When set, the PDIS memory buffer is configured for use by supervisor software.  PDISS is used to select the trace write privilege and translation mechanism used by AMB, see Section 1.5 of the https://github.com/riscv/self-hosted-trace/releases[Self-hosted Trace Specification].

If `mpdisctl`.MEM is hardwired to 0, PDISS is hardwired to 0 as well.
...
When `mpdisctl`.MEM=0, sample records are written to a set of indirect CSRs accessed using Sscsrind.  See <<samprec>> for details on the sample data.  When `mpdisctl`.MEM is hardwired to 1, these registers are not implemented.

These registers are populated only when a sampled instruction is a qualified sample.  The registers are mapped to record fields as follows.
...
PDIS Sample Data registers are implemented only if `mpdisctl`.MEM is not hardwired to 1.
...
If latency filtering is not supported, `spdisfilter` must be read-only 0.  Otherwise, THRESH and INV must remain writable.  LATSEL may optionally be hardwired to 0, implying that only filtering by "Total latency" is supported.

==== Supervisor PDIS AMB Control and Status (`spdisambcs`)

The `spdisambcs` register is the PDIS instatiation of the `sambcs` register defined by the Ssamb extension, see Section 1.1.1 of the https://github.com/riscv/self-hosted-trace/releases[Self-hosted Trace Specification].  It is implemented only if `mpdisctl`.MEM is not hardwired to 0.  See <<membuff>> for PDIS Memory Buffer details.

==== Supervisor PDIS AMB Address (`spdisambaddr`)

The `spdisambaddr` register is the PDIS instatiation of the `sambaddr` register defined in the Ssamb specification, see Section 1.1.2 of the https://github.com/riscv/self-hosted-trace/releases[Self-hosted Trace Specification].  It is implemented only if `mpdisctl`.MEM is not hardwired to 0.  See <<membuff>> for PDIS Memory Buffer details.

For RV64 implementations, bits 6:0 are hardwired to 0, while for RV32 implementations, bits 5:0 are hardwired to 0.  This ensures that records are aligned to their size, see <<samprec>> for details.
...
NOTE: _Before PDIS, `scountinhibit` provided access to `mcountinhibit` & `mcounteren`, allowing S-mode to inhibit only delegated counters.  Since PDIS isn't delegated through `mcounteren` (bit 1 controls read access to `time`), the PDIS bit has to be handled differently._
...
If `mpdisctl`.MEM is hardwired to 0, PDISV is hardwired to 0 as well.
...
NOTE: _If the implementation cannot provide the guest physical address (GPA) for sampled loads and/or stores, perhaps because the GPA is not stored in the TLB, the `vspdisctl`.PA bit should be hardwired to 0._
...
When Smstateen is implemented, the `mstateen0`.PDIS bit controls access to PDIS register state from privilege modes less privileged than M-mode.  When `mstateen0`.PDIS=1, supervisor level accesses to PDIS register state behave as described in <<CSRs>> above.  When `mstateen0`.PDIS=0 and the privilege mode is less privileged than M-mode, attempts to access `spdis*` or `vspdisctl` raise an illegal-instruction exception, while `scountovf`[1], `scountinhibit`[1], and `henvcfg`.PDISV are read-only 0.
...
[[postret-st]]
==== Post-retirement Stores

In some implementations, a store may be allowed to retire before it has fully completed its execution.  In this text, retire implies the uop is deallocated from the ROB, and hence younger instructions/uops are allowed to commit their state.  Such a store may retire when it is non-speculative (that is, neither the store nor an older instruction will trap), but before it has performed a read-for-ownership (RFO) and subsequent memory write.  Such implementations should extend sample active until the sampled store has fully completed (retired, RFO completed, and write performed) and set `mpdisctl`.FNC=1 for the sample, to ensure that software record collection waits for all sample data to be recorded.

WARNING: _What to do about late-filtering w.r.t. post-retirement stores?  Better to wait for the store to complete and apply filters, at the expense of more interrupt skid?  Or keep the interrupt precise (after store retirement, but before store write execution), and skip filtering?_
...
| ADR1V | When set, indicates that a valid address was captured in pdisadr1.  See <<pdisadr1>>.
| ADR2V | When set, indicates that a valid address was captured in pdisadr2.  See <<pdisadr2>>.
...
0: FLUSH field is not implemented
...
| ICITV | When set, the ITMISS and ICMISS fields are implemented.
...
The TYPE, FLUSHED, PARTIAL, FUSED, FMT, and SFMT fields are required, while other fields are optional.  For some implementations, PARTIAL and/or FUSED may always report 0.  Unimplemented fields should always report 0.

Additional sample data fields are included in PDIS Header & Events when the type is load, store, load & store, or control transfer instruction.

[[pdishdrldst]]
===== PDIS Header and Events for Load and Store Instructions

For load, store, and load & store instructions, the following sample data is collected in pdishdrev.
...
| LLMISS | Data request missed the last-level cache that can be tracked by the core, which may not be the last-level cache in the system.
| DSRC | Data source.  Encodings for this field are implementation-defined, though 0 implies that DSRC is not implemented.
| Others | Defined in <<recevents>> above.
|===

All fields are required save for DSRC.

===== PDIS Header and Events for Control Transfer Instructions
...
The MISPRED bit is required, the control transfer type bits are optional but are either all implemented or none.  When the transfer type bits are implemented, exactly one of them will be set for all samples with pdishdrev.TYPE=4.
...
If at least one virtual address, aside from the PC, is captured for this sample, it is held in pdisadr1.  Capturing an address (aside from PC) is optional; `mpdisctl`.ADR1V is set to 1 when a virtual address is captured for this sample, and set to 0 when no address is captured or when pdisadr1 is not implemented.
...
If an additional address is captured for this sample, aside from the PC and pdisadr1, it is held in pdisadr2.  Capturing a second address (aside from PC) is optional; `mpdisctl`.ADR2V is set to 1 when a second address is captured for this sample, and set to 0 when no second address is captured or when pdisadr2 is not implemented.
...
NOTE: _The virtual address of the access may not be resolved by the time the instruction is flushed.  In such cases, no pdisadr2 is captured._

If the instruction was not flushed and the instruction is of type load, store, or load & store, then:

* if the sampled instruction executed in VS-mode or VU-mode and `vspdisctl`.PA=1, pdisadr2 holds the data guest physical address, shifted right by 2, to which the virtual address in pdisadr1 is mapped;
* otherwise, if `mpdisctl`.PA=1, pdisadr2 holds the data physical address, shifted right by 2, to which the virtual address in pdisadr1 is mapped.

NOTE: _A physical address or guest physical address written to pdisadr2 is shifted right by 2 bits to accommodate addresses wider than the current XLEN. The Sv32 page-based virtual-memory scheme described in Section 12.3 supports 34-bit physical addresses for RV32, so pdisadr2 must support addresses wider than XLEN for RV32. This shift-by-2 encoding of physical and guest physical addresses matches the encoding of physical addresses in PMP address registers (Section 3.7) and in page table entries (Section 12.3, Section 12.4, Section 12.5, and Section 12.6).  If the least-significant two bits are needed, these bits are the same as the least-significant two bits of the virtual address in pdisadr1._

If an implementation does not support capturing physical addresses, `mpidsctl`.PA must be hardwired to 0.  If an implementation does not support capturing guest-physical addresses, `vspdisctl`.PA must be hardwired to 0.
...
A custom PDIS extension that alters standard PDIS behavior must be associated with a non-zero value within the designated custom bits in `mpdisctl`.  When the custom bits hold a non-zero value that enables a custom extension, the extension may alter standard PDIS behavior, which may include redefining standard record fields.

[appendix]
[[sampmodes]]
=== Sampling Modes and Record Collection
...
. Inhibit the PDIS counter by setting `__x__countinhibit[1]`=1.
** This avoids counting or sampling handler instructions, and avoids risk of the PDIS sample data registers being modified as the record is being collected.
** Because LCOFIs are used for Zihpm counter overflows as well, the handler may simply write all 1s to `__x__countinhibit`.
...
*** If `mpdisctl`.FNC=1, perform a FENCE to ensure any sampled store has completed, and all sample data has been captured.
...
** Clear `mpdisctl`.OF and `mpdisctl`.FNC
. Clear `__x__ip`.LCOFI
...
See <<lambi-handling>> for general guidance on handling LAMBIs.  Below is additional, PDIS-specific guidance for a LAMBI handler running in privilege mode _x_.

In step 1, if `spdisambcs`.BMI=1, inhibiting the PDIS counter by setting `__x__countinhibit[1]`=1 serves to disable the PDIS data source.

In step 3, if `spdisambcs`.BMI=1, follow the guidance in <<lcofi-collection>> above regarding `mpdisctl`.FNC when collecting PDIS record(s).

In step 4, if `spdisambcs`.BMI was set in step 1, software must clear `__x__countinhibit[1]` if continued sampling is desired.
