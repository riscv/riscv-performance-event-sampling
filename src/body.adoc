...
| RSTATE | The state of the *Retire* stage in the cycle the sample is taken. +
 `00` signifies that the core committed at least one instruction +
...
| CAUSE| This field provides further information about the cause of the retire stage being empty, e.g., the (combinations of) performance event(s) that caused no instructions to retire. This specification recommends fixing the first set of CAUSE bits to be I$miss, ITLBmiss, Branch mispredict, D$miss, DTLBmiss.
|====
