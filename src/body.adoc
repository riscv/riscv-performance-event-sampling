...
| MEM | Read-only bit that, when set, indicates that sample records (<<samprec>>) are written to the PDIS Memory Buffer (<<membuff>>).  When 0, sample records are instead written to `spdisdata__i__` registers (<<dataregs>>).
