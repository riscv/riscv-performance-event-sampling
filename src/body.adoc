...
PDIS Sample Data registers are implemented only if `mpdisctl`.MEM is not hardcoded to 1.
...
If latency filtering is not supported, `spdisfilter` must be read-only 0.  Otherwise, THRESH and INV must remain writable.  LATSEL may optionally be hardcoded to 0, implying that only filtering by "Total latency" is supported.
...
If an implementation does not support capturing physical addresses, `mpidsctl`.PA must be hardcoded to 0.  If an implementation does not support capturing guest-physical addresses, `vspdisctl`.PA must be hardcoded to 0.
